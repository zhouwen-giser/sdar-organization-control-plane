...
- Public route count: **55**
- Unique public paths: **54**
- Root alias: `/` redirects through the same product experience as `/overview`.
- Unknown paths render a domain-complete 404 state and do not fall back to Overview.
- Hash routes support direct refresh in a static deployment.
- Chromium Smoke: **53/53 passed**, no unhandled exception or console error.
