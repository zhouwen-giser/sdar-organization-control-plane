...
export function NodeHealthPage() {
  const { node, telemetry, records } = useGatewaySnapshot();
  return <div className="page-stack"><section className="page-intro"><div><span className="eyebrow">GET /api/v1/node/health</span><h2>节点健康与收敛</h2><p>聚合控制后台、Runtime、Catalog 和 Telemetry Export 的当前健康事实。</p></div><Badge value={node.health.status} /></section>
    <div className="metric-grid four"><MetricCard icon={<HeartPulse />} label="节点状态" value={node.health.status} detail="当前综合判断" tone={node.health.status === 'healthy' ? 'success' : 'warning'} /><MetricCard icon={<Activity />} label="活动任务" value={node.health.activeTasks} detail={`${records.task.length} 个任务投影`} /><MetricCard icon={<Database />} label="待收敛配置" value={records.configuration.filter((item) => item.fields.convergence !== 'converged').length} detail="Desired ≠ Observed" tone="warning" /><MetricCard icon={<Radio />} label="待发送事实" value={telemetry.status.pendingRecords} detail="Telemetry Outbox" tone="info" /></div>
