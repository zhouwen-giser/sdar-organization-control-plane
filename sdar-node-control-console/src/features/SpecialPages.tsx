...
export function TelemetryPage({ edit = false }: { edit?: boolean }) {
  const snapshot = useGatewaySnapshot();
  const { execute, canInvoke } = useConsole();
  const [tab, setTab] = useState(edit ? 'edit' : 'overview');
  const [form, setForm] = useState({ endpointRef: String(snapshot.telemetry.configuration.fields.endpointRef), credentialRef: String(snapshot.telemetry.configuration.fields.credentialRef), sourceId: String(snapshot.telemetry.configuration.fields.sourceId), applyMode: String(snapshot.telemetry.configuration.fields.applyMode) });
  const [reason, setReason] = useState('');
  const [done, setDone] = useState(false);
  const createOperation = getOperation('createTelemetryExportRevision');
  const submit = async () => { await execute({ operation: createOperation, target: { type: 'telemetryExport', id: snapshot.telemetry.configuration.id, revision: Number(snapshot.telemetry.configuration.revision) }, expectedRevision: Number(snapshot.telemetry.configuration.revision), reason, idempotencyKey: `telemetry-revision-${Date.now()}`, payload: { ...snapshot.telemetry.configuration.fields, ...form } }); setDone(true); };
  return <div className="page-stack"><section className="detail-hero"><div className="detail-title"><span className="eyebrow">Telemetry Export Contract 1.0.0</span><div><h2>Telemetry Export</h2><Badge value={snapshot.telemetry.status.status} /></div><p>配置事实出口和本地 Delivery State；不提供遥测查询、任务时间线或 ClickHouse 代理。</p></div><div className="hero-actions"><OperationAction operationId="testTelemetryExportConnection" target={{ type: 'telemetryExport', id: snapshot.telemetry.configuration.id, revision: Number(snapshot.telemetry.configuration.revision) }} /><Button variant="primary" onClick={() => { setTab('edit'); if (!edit) navigate('/telemetry-export/edit'); }}>创建 Revision</Button></div></section>
    <Callout title="禁止 Telemetry Query" tone="warning">SDAR 只管理 Export 配置和本地投递状态。遥测数据由独立 Telemetry Platform 查询，控制台不会提供 /telemetry/query。</Callout>
    <div className="metric-grid four"><MetricCard label="出口状态" value={snapshot.telemetry.status.status} detail={`Active Revision ${snapshot.telemetry.status.activeRevision}`} tone="success" /><MetricCard label="Pending Records" value={snapshot.telemetry.status.pendingRecords} detail={`Oldest ${formatTime(snapshot.telemetry.status.oldestPendingAt ?? snapshot.telemetry.status.observedAt)}`} /><MetricCard label="ACK Sequence" value={snapshot.telemetry.status.lastAcknowledgedSequence} detail={formatTime(snapshot.telemetry.status.lastAcknowledgedAt)} /><MetricCard label="WAL 策略" value="fsync" detail="持久化后再 ACK" tone="info" /></div>
    <Tabs active={tab} onChange={setTab} tabs={[{ id: 'overview', label: '配置与状态' }, { id: 'edit', label: 'Revision 草稿' }, { id: 'delivery', label: 'Delivery 语义' }]} />
    {tab === 'overview' && <div className="detail-grid"><Panel title="出口配置" className="span-2"><DefinitionList fields={snapshot.telemetry.configuration.fields} /></Panel><Panel title="本地 Delivery State"><DefinitionList fields={snapshot.telemetry.status as never} /></Panel></div>}
    {tab === 'edit' && <Panel title="创建 Telemetry Export Revision" subtitle="Secret 只能使用 credentialRef。">{done ? <div className="completion-inline"><CheckCircle2 /><div><strong>Revision 草稿已创建</strong><p>下一步执行 Validate 和 Publish；Publish 后仍需 Runtime ACK。</p></div></div> : <><div className="form-grid"><label className="form-field full"><span>Endpoint Ref</span><input value={form.endpointRef} onChange={(event) => setForm({ ...form, endpointRef: event.target.value })} /></label><label className="form-field full"><span>Credential Ref</span><input value={form.credentialRef} onChange={(event) => setForm({ ...form, credentialRef: event.target.value })} /></label><label className="form-field"><span>Source ID</span><input value={form.sourceId} onChange={(event) => setForm({ ...form, sourceId: event.target.value })} /></label><label className="form-field"><span>Apply Mode</span><select value={form.applyMode} onChange={(event) => setForm({ ...form, applyMode: event.target.value })}><option>hot_reload</option><option>reconnect_required</option><option>restart_required</option></select></label><label className="form-field full"><span>变更原因</span><textarea value={reason} onChange={(event) => setReason(event.target.value)} /></label></div><div className="form-footer"><Button onClick={() => navigate('/telemetry-export')}>取消</Button><Button variant="primary" disabled={!canInvoke(createOperation) || reason.trim().length < 5 || !form.credentialRef.startsWith('secret://')} onClick={submit}>创建 Revision</Button></div></>}</Panel>}
    {tab === 'delivery' && <Panel title="出口可靠性模型"><div className="timeline"><div className="complete"><span>1</span><div><strong>Runtime 产生正式事实</strong><p>事实先进入事务 Outbox / WAL。</p></div></div><div className="complete"><span>2</span><div><strong>WAL fsync</strong><p>记录持久化完成后才允许向上游 ACK。</p></div></div><div><span>3</span><div><strong>批量投递与重试</strong><p>按 Batch、Retry、TLS 与 CredentialRef 策略投递。</p></div></div><div><span>4</span><div><strong>Telemetry Platform 独立查询</strong><p>SDAR 不代理遥测查询。</p></div></div></div></Panel>}
  </div>;
}
