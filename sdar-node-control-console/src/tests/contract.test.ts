...
describe('frozen contract integration', () => {
  ...
  it('maps all 85 public operations exactly once', () => {
    expect(CONTRACT_OPERATIONS).toHaveLength(85);
    expect(new Set(CONTRACT_OPERATIONS.map((operation) => operation.operationId)).size).toBe(85);
  ...
  it('assigns a stable scope to every operation', () => {
    for (const operation of CONTRACT_OPERATIONS) expect(requiredScope(operation)).toMatch(/^[a-z0-9_]+\.(read|write|manage|control)$/);
  });
});
